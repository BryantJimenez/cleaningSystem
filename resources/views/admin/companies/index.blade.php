...
@section('title', 'Lista de companias')
							...
							<table class="table table-hover table-export">
								...
								<tbody>
									...
									<tr>
										<td>{{ $num++ }}</td>
										<td>{{ $company->name }}</td>
										<td>{!! typeCompany($company->type) !!}</td>
										<td>{!! state($company->state) !!}</td>
										<td>
											<div class="btn-group" role="group">
												<a href="{{ route('companias.edit', ['slug' => $company->slug]) }}" class="btn btn-info btn-sm bs-tooltip" title="Editar"><i class="fa fa-edit"></i></a>
												@if($company->state==1)
												<button type="button" class="btn btn-danger btn-sm bs-tooltip" title="Desactivar" onclick="deactiveCompany('{{ $company->slug }}')"><i class="fa fa-power-off"></i></button>
												@else
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title">¿Estás seguro de que quieres desactivar esta Compañía?</h5>
...
<div class="modal fade" id="activeCompany" tabindex="-1" role="dialog" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title">¿Estás seguro de que quieres activar esta Compañía?</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
